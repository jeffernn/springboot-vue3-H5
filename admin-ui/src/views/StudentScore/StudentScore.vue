    ...
    <el-main>
      <el-card>
        ...
        <el-table :data="tableData" border stripe style="width: 100%">
          ...
          <el-table-column prop="totalPoint" label="累计积分" width="150"></el-table-column>
    ...
    >
      <el-form ref="formRef" :model="currentRecord" label-width="120px" label-position="right">
        ...
        <el-form-item label="累计积分">
          <el-input-number v-model="currentRecord.totalPoint" :min="0" />
        </el-form-item>
        <el-form-item>
...
const getPage = async () => {
  try {
    const res = await studentScoreApi.queryPage(queryParams.value)
    if (res && res.records) {
...
const submitForm = async () => {
  try {
    await studentScoreApi.updateStudentScore(currentRecord.value)
...
const handleDelete = async (row) => {
  try {
    ...
    await studentScoreApi.deleteStudentScore({ id: row.id })
